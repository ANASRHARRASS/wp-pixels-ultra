...
function wpu_ingest_handler( $request ) {
	...
	$params      = is_array( $body['params'] ) ? $body['params'] : array();
	...
	if ( empty( $provider_id ) ) {
		return new WP_REST_Response( array( 'error' => 'provider_id is required' ), 400 );
	}
	...
	if ( ! class_exists( 'WPU_Provider_Manager' ) ) {
		return new WP_REST_Response( array( 'error' => 'provider manager not available' ), 500 );
	}

	$result = WPU_Provider_Manager::fetch_from_provider( $provider_id, $params );
	if ( is_wp_error( $result ) ) {
		$code = 502;
		if ( in_array( $result->get_error_code(), array( 'rate_limited' ), true ) ) {
			$code = 429;
		}
