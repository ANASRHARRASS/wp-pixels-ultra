...
class WPU_Provider_Manager {
	public static function load_providers() {
		$list = get_option( 'wpu_providers_config', array() );
		$out  = array();
	...
	public static function fetch_from_provider( $id, $params = array() ) {
		...
		if ( $code >= 400 ) {
			return new WP_Error( 'remote_error', 'Provider returned HTTP ' . $code, array( 'body' => $body ) );
		}
