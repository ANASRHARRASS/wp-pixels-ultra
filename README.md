...
- 🔐 **Deterministic Event Idempotency**: Added `event_id` column and generation logic to prevent duplicate queue entries
- 🧱 **Schema Upgrade Handler**: Automatic backfill of `event_id` for existing installs via `UP_Upgrade`
- 🧪 **Expanded CI Tooling**: Added PHPStan, PHPCS, and PHP syntax workflows for higher code quality
- 🛠️ **Idempotent Ingest**: REST ingest accepts optional client `event_id` and returns it in response
- 📦 **Composer Dev Setup**: Introduced `composer.json` for static analysis dependencies
